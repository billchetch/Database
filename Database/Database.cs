...
namespace Chetch.Database
{
    ...
    public class DBRow : Dictionary<String, Object>, IDBRow
    {
        ...
        virtual public String GenerateParamString(String key, Object value, bool asLiterals = false, Func<String, Object, bool, String> paramStringFunc = null)
        {
            ...
            else
            {
                if (paramStringFunc == null)
                {
                    ...
                    {
                        v = Database.asString((DateTime)value, Database.DATE_TIME_FORMAT);
                    }
                    else
                    {
        ...
        {
            return (byte)System.Convert.ToInt16(GetValue<int>(fieldName, defaultValue));
        }
    ...
    public class SysInfo : DBRow
    {
        ...
        override public void AddField(String fieldName, Object fieldValue)
        {
            ...
            switch (fieldName)
            {
                case "data_value":
                    if (fieldValue != null)
                    {
                        DataValue = JsonSerializer.Deserialize<Dictionary<String, Object>>(fieldValue.ToString());
                    }
                    break;
        ...
        {
            this["data_value"] = JsonSerializer.Serialize(DataValue);
        }
    }
    ...
    public class Database : IDisposable
    {
        ...
        static public D Create<D>(String server, String database, String uid, String password) where D : Database, new()
        {
            var db = new D();
        ...
        public Database(String server, String database, String uid, String password, String salt)
        {
            var pwd = BasicEncryption.Decrypt(password, salt);
            Configure(server, database, uid, pwd);
            Initialize();
        }
